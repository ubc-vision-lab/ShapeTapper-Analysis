...
class Shape :
    ...
    def __loadImg(self) :
        ...
        if self.img is None : 
            ...
            self.dims = self.img.shape # store image dimensions for quick access
    ...
    def __loadRefObjs(self) :
        # Create file name for reference object MAT file
        ...
        # If successful, store reference object points as NumPy arrays
    ...
    def fitMedialAxisFrom(self, shape_from) :
        ...
        # Use fit_ro from map_shape_pairs.py to find best fit (returned as Transform object)
        transform = fit_ro(shape_from.dims, shape_from.medial_axis, self.dims, self.medial_axis, usefast=0)
        
        # Add to dictionary of transforms
        self.fitted_transforms.update({shape_from.name : transform})