...
if __name__ == '__main__':
	img_name = "blake_01"
	shape_img = "images_black/" + img_name + ".png"
	divvy = 18


	generated_results_file = "./Patient_2/" + img_name + "_Patient_2_generated_results.mat"
	...
	results_mat = sio.loadmat(generated_results_file)
	results = results_mat['img_datasets']
	filler = np.transpose([np.zeros(np.shape(results)[1],dtype=int)])
	centroid_data = []
	medaxis_data = []
	for result in results:
		...
		for touch in result_filled:
			min_dist = 9999
			centroid_dists += [assist.distance(touch,centroid)]
		...
		medaxis_var = np.sum(np.power(min_dists, 2))/(np.size(min_dists)-1)
		# medaxis_mse = np.mean(np.power(min_dists, 2))
		# medaxis_std = np.sqrt(medaxis_var)
		medaxis_amd = np.mean(min_dists)
		medaxis_data += [medaxis_var, medaxis_amd]

		centroid_var = np.sum(np.power(centroid_dists, 2))/(np.size(centroid_dists)-1)
		centroid_amd = np.mean(centroid_dists)
		centroid_data += [centroid_var, centroid_amd]

	print medaxis_data
	print np.shape(medaxis_data)
	print centroid_data
